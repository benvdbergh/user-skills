...
const { wantsHelp, printHelpAndExit } = require('./lib/cli-help.cjs');

const USAGE = [
  'sync-brand-to-tokens.cjs — Sync docs/brand-guidelines.md colors to design tokens',
  '',
  'Usage:',
  '  bun scripts/sync-brand-to-tokens.cjs [--dry-run]',
  '',
  'Options:',
  '  --dry-run   Preview changes without writing files',
  '  --help      Show this help',
  '',
  'Requires design-system skill (generate-tokens.cjs) in the skills repo or .claude/skills layout.',
].join('\n');
...
function resolveGenerateTokensScript() {
  const candidates = [
    path.join(__dirname, '..', '..', 'design-system', 'scripts', 'generate-tokens.cjs'),
    path.resolve(process.cwd(), '.claude/skills/design-system/scripts/generate-tokens.cjs'),
  ];
...
function main() {
  ...
  if (wantsHelp(args)) printHelpAndExit(USAGE.split('\n'));
  ...
  // Regenerate CSS via design-system
  const generateScript = resolveGenerateTokensScript();
  if (generateScript) {
    try {
      execSync(
        `bun "${generateScript}" --config ${DESIGN_TOKENS_JSON} -o ${DESIGN_TOKENS_CSS}`,
    ...
    console.error('⚠️  design-system generate-tokens.cjs not found; updated JSON only.');
  }
