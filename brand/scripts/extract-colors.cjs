...
const USAGE = [
  "extract-colors.cjs — Show brand palette or guide image color extraction",
  "",
  "Usage:",
  "  bun scripts/extract-colors.cjs --palette",
  "  bun scripts/extract-colors.cjs <image-path> [--brand-file <path>] [--json]",
  "",
  "Options:",
  "  --palette       Print palette from brand guidelines",
  "  --brand-file    Path to guidelines (default: docs/brand-guidelines.md)",
  "  --json          JSON output",
  "  --help          Show this help",
].join("\n");
  ...
  if (showPalette || !imagePath) {
    if (jsonOutput) {
      ...
      if (!imagePath) {
        ...
        console.log("  bun scripts/extract-colors.cjs <image-path>");
