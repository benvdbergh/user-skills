...
export function SkillDetailPanel({
  ...
  const healthSeverity =
    header.health.status === "error" || header.health.status === "warning"
      ? header.health.status
      : undefined;
  const healthLink =
    header.health.findings > 0
      ? buildHealthPath({
    ...
    >
      <header className="sl-detail-header">
        ...
        <div className="sl-detail-actions">
          ...
          {healthLink && (
            <Link to={healthLink} className="sl-btn sl-btn-ghost">
              <ShellIcon name="health" size={14} />
              View in Health
            </Link>
          )}
          {source?.isLinkable ? (
