...
Create a task list or plan document in the project directory (e.g. `plans/social-photos-{date}.md`) to track the deliverables. Break down into:
...
1. Activate **`brand`** — Extract brand colors, fonts, voice from the user's project
2. Activate **`design-system`** — Get design tokens (spacing, typography scale, color palette)
3. Activate **one of** `ui-ux-pro-max` or `frontend-design` — for layout, hierarchy, visual balance (vary per run for design variety)
...
Organize the exported files into a named folder (e.g. `social-exports/{platform}/`) and create a summary report at `plans/reports/social-photos-summary.md`.
