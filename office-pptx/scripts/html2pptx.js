 ...
 *
 * Vendored from claude-office-skills. See references/html2pptx.md for usage.
 */
...
async function extractSlideData(page) {
  return await page.evaluate(() => {
    ...
    if (brTags.length > 0) {
      errors.push('<br> tags are not allowed. Use separate text elements per line instead.');
    }

    if (bgImage && (bgImage.includes('linear-gradient') || bgImage.includes('radial-gradient'))) {
      errors.push(
        'CSS gradients are not supported. Rasterize gradients as PNG images first, then reference with background-image.'
      );
    }
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (textTags.includes(el.tagName)) {
        ...
        if (hasBg || hasBorder || hasShadow) {
          errors.push(
            ...
            'Backgrounds/borders/shadows are only supported on <div> elements.'
          );
          return;
        }
      }
      ...
      if (el.className && el.className.includes('placeholder')) {
        ...
        if (rect.width === 0 || rect.height === 0) {
          errors.push(`Placeholder "${el.id || 'unnamed'}" has ${rect.width === 0 ? 'width: 0' : 'height: 0'}.`);
        } else {
          placeholders.push({
          ...
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            if (text) {
              errors.push(
                ...
                'Wrap all text in <p>/<h1>-<h6>/<ul>/<ol>.'
              );
            }
          }
        }
      ...
      if (el.tagName !== 'LI' && /^[•\-\*▪▸○●◆◇■□]\s/.test(text.trimStart())) {
        errors.push(`Text element <${el.tagName.toLowerCase()}> starts with a manual bullet symbol. Use <ul>/<ol>.`);
        return;
      }
      ...
      if (hasFormatting) {
        ...
        const transformedRuns = runs.map(run => ({ ...run, text: applyTextTransform(run.text, textTransform) }));
...
async function html2pptx(htmlFile, pres, options = {}) {
  const { tmpDir = process.env.TMPDIR || process.env.TEMP || '/tmp', slide = null } = options;
